...
public:
	...
	virtual bool		IsExited() = 0;													// 用于确认网络线程是否已经正常的退出
	virtual void		Release() = 0;													// 释放内存空间
	virtual bool		ConnectTo(char *pstrAddr, const unsigned short usPort) = 0;		// 连接服务器
	virtual bool		ConnectToUrl(char *pstrAddr, const unsigned short usPort) = 0;	// 连接服务器
	virtual void		ShutDown() = 0;
};

class IMultiClientNetwork
{
public:
	virtual void		Stop() = 0;																					// 退出时调用（用于网络线程退出）
	virtual bool		IsExited() = 0;																				// 用于确认网络线程是否已经正常的退出
